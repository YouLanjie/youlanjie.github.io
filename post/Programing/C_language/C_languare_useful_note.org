...
| =\e[6n=                       | 查询光标位置                         | 从标准输入读取结果,格式: =\e[<r>;<c>R=       |
#+begin_quote
...
| =\e7=, =\e[s= | *保存当前光标位置和属性*              | 推荐用前者                            |
| =\e8=, =\e[u= | *恢复之前保存的光标位置和属性*        | 推荐用前者                                |
| =\eM=         | 反转的索引 (Reverse Index) - 上滚一行 | =printf "\eM"=                                |
** 非标准ANSI序列控制字符
- 隐藏光标： =\033[?25l=
- 显示光标： =\033[?25h=
- 切换备用屏幕缓冲区： =\033[?47h= =\033[?1049h= (你ncurses“保存历史记录”的实现原理)
- 切回主屏幕缓冲区： =\033[?47l= =\033[?1049l=
* Linux Signal信号
#+begin_quote
