...
- 普通新闻\\
  ...
  外会有一些独有的新闻信息。
- 软件包在仓库的变动\\
  可以访问archlinux软件仓库的git仓库： [[https://gitlab.archlinux.org/archlinux/packaging/state][/官方仓库链接/]]，[[https://github.com/archlinux/state][/github镜像/]]，建议访问官
  方而非github，一般github更慢

  这一点主要是可以在你pacman更新更着更着发现有个软件包“消失了”，变成了需要从
  aur下载了，就可以去git仓库然后搜索想找的软件名，选择搜索commit，按照时间排序，
  就可以看到那个软件包是不是被 *Removed* 了。当然如果包还在那么在code里面应该会
  有搜索结果的。

  当然社区仓库archlinuxcn也是同理，git仓库链接：[[https://github.com/archlinuxcn/repo][只有github的链接]]
  ...
  #+begin_quote
  不过凡事必有因，archlinuxcn的提交有一个可追踪的因果链，根据Search Commit可找到
  到弃置请求的issue，再根据链接找到真正导致弃置的原因。
