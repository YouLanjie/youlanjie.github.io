...
的命令开启清华镜像源:
#+begin_src shell
  sed -i "/tuna/s/#//" /etc/pacman.d/mirrorlist
#+end_src
...
** 使用wayland
- kde\\
  见上面x11部分，kde目前(2025.08.01)已经是默认使用wayland会话而不使用x11了
** 中文字体
#+begin_src bash
pacamn -S wqy-zenhei
#+end_src
其他我安装的字体：
#+begin_src bash
  pacamn -S ttf-liberation ttf-fira-code ttf-fira-mono ttf-nerd-fonts-symbols-mono ttf-jetbrains-mono
#+end_src
** 使用AUR
使用AUR助手yay:
#+begin_src bash
