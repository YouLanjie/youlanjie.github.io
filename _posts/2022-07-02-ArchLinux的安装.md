...
1. 从[官网](https://archlinux.org/download/)上下载ArchLinux的安装镜像（以格式一般为*archlinux-YYYY.MM.DD-x86\_64.iso*）（下滑找到 **China** 分栏有国内镜像服务器源，速度会快很多，推荐[清华云tuna.tsinghua.edu.cn](https://mirrors.tuna.tsinghua.edu.cn/archlinux/iso/latest/)与[阿里云aliyun.com](https://mirrors.aliyun.com/archlinux/iso/latest/)）

2. windows下则使用任意一款U盘拷录软件（自行百度）拷录镜像文件到U盘（注意，被拷录的U盘的所有数据都会丢失，请备份好资料后再执行操作）。而Linux下则可以（使用root权限）使用命令
```bash
dd if=你的镜像文件名称 of=/dev/sdX bs=512    #sdX为要拷录的U盘设备，实际中应为sda或sdb...
```
拷录镜像文件到U盘上（Linux的磁盘管理机制请自行百度）

> **除非你能够同时查看教程则直接重启，否则建议先记下下面的安装过程较好**
...
> 个人办法：其实如果在开机时留意一下弹出过的菜单界面就能知道你的是什么引导方式。BIOS引导的长这样：
![BIOS](https://youlanjie.github.io/img/in-post/2022-07-02_02/2022-07-02_02-02.png)
而EFI的长这样：
![EFI](https://youlanjie.github.io/img/in-post/2022-07-02_02/2022-07-02_02-01.png)
...
操作演示图片：
![操作演示](https://youlanjie.github.io/img/in-post/2022-07-02_02/2022-07-02_02-07.png)
...
按下 **/** 并输入 **China** 并回车，跳转到中国服务器的镜像源列表。在列表上方一列输入 `dgg` 删除列表上面的内容，在列表下方一列输入 `dG` 移除列表以下内容，建议将清华、阿里云的源以 `dd` 删除一行，再 `p` 粘贴的操作放在文件最前面几列，最后输入 `:w` 回车保存 `:q` 回车退出。（可以合起来为 `:wq` 再回车直接保存与退出）。操作过程演示：
<iframe src="https://youlanjie.github.io/img/in-post/2022-07-02_02/2022-07-02_02-08.webm" width="512px" height="384px"></iframe>
...
![虚拟机——空白](https://youlanjie.github.io/img/in-post/2022-07-02_02/2022-07-02_02-03.png)
...
格式化分区，对应的看下面 [分区建议](https://youlanjie.github.io/2022/07/02/ArchLinux%E7%9A%84%E5%AE%89%E8%A3%85/#%E5%88%86%E5%8C%BA%E5%BB%BA%E8%AE%AE) 表格
